    ...
    public class BinaryTree<TKey, TValue> where TKey : IComparable<TKey>
    {
        ...
        private BinaryTreeNode<TKey, TValue> AddBalanced(TKey[] keys, TValue[] values, int start, int end)
        {
            ...
            else
            {
                ...
                var node = new BinaryTreeNode<TKey, TValue>(keys[middle], values[middle]);
                node.LeftChild = AddBalanced(keys, values, start, middle - 1);
                node.RightChild = AddBalanced(keys, values, middle + 1, end);
                return node;
            }
        }
